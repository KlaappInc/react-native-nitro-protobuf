...
test('native round-trip encode/decode', (t) => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '  map<string, int32> m_string_int = 21;',
      '  map<int32, string> m_int_string = 22;',
      '  oneof choice { string o_string = 23; int32 o_int = 24; }',
      '}',
    ].join('\n'),
    'utf8'
  )
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '  }',
      '',
      '  // Every proto type in one message round-trips.',
      '  const MessageInfo* atInfo = getMessageInfo("acme.AllTypes");',
      '  expect(atInfo != nullptr, "AllTypes message available");',
      '  if (atInfo != nullptr) {',
      '    auto a = AnyMap::make();',
      '    a->setDouble("f_double", 1.5);',
      '    a->setDouble("f_float", 2.5);',
      '    a->setDouble("f_int32", -7);',
      '    a->setString("f_int64", "-9007199254740993");',
      '    a->setDouble("f_uint32", 42);',
      '    a->setString("f_uint64", "18446744073709551615");',
      '    a->setDouble("f_sint32", -8);',
      '    a->setString("f_sint64", "-12345678901234");',
      '    a->setDouble("f_fixed32", 100);',
      '    a->setString("f_fixed64", "9999999999");',
      '    a->setDouble("f_sfixed32", -100);',
      '    a->setString("f_sfixed64", "-9999999999");',
      '    a->setBoolean("f_bool", true);',
      '    a->setString("f_string", "hi");',
      '    a->setArray("f_bytes", AnyArray{AnyValue(1.0), AnyValue(2.0), AnyValue(255.0)});',
      '    a->setDouble("f_enum", 2);',
      '    a->setObject("f_msg", AnyObject{{"street", AnyValue(std::string("Main"))}, {"zip", AnyValue(5.0)}});',
      '    a->setArray("r_int32", AnyArray{AnyValue(1.0), AnyValue(2.0), AnyValue(3.0)});',
      '    a->setObject("m_string_int", AnyObject{{"x", AnyValue(9.0)}});',
      '    a->setObject("m_int_string", AnyObject{{"7", AnyValue(std::string("seven"))}});',
      '    a->setString("o_string", "pick");',
      '    auto d = decodeMessage(*atInfo, encodeMessage(*atInfo, a));',
      '    expectNear(d->getDouble("f_double"), 1.5, 1e-9, "AllTypes double");',
      '    expectNear(d->getDouble("f_float"), 2.5, 1e-6, "AllTypes float");',
      '    expect(d->getDouble("f_int32") == -7, "AllTypes int32");',
      '    expect(d->getString("f_int64") == "-9007199254740993", "AllTypes int64");',
      '    expect(d->getDouble("f_uint32") == 42, "AllTypes uint32");',
      '    expect(d->getString("f_uint64") == "18446744073709551615", "AllTypes uint64");',
      '    expect(d->getDouble("f_sint32") == -8, "AllTypes sint32");',
      '    expect(d->getString("f_sint64") == "-12345678901234", "AllTypes sint64");',
      '    expect(d->getDouble("f_fixed32") == 100, "AllTypes fixed32");',
      '    expect(d->getString("f_fixed64") == "9999999999", "AllTypes fixed64");',
      '    expect(d->getDouble("f_sfixed32") == -100, "AllTypes sfixed32");',
      '    expect(d->getString("f_sfixed64") == "-9999999999", "AllTypes sfixed64");',
      '    expect(d->getBoolean("f_bool") == true, "AllTypes bool");',
      '    expect(d->getString("f_string") == "hi", "AllTypes string");',
      '    expect(d->getString("f_bytes") == "AQL/", "AllTypes bytes base64");',
      '    expect(d->getDouble("f_enum") == 2, "AllTypes enum");',
      '    expect(std::get<std::string>(d->getObject("f_msg").at("street")) == "Main", "AllTypes message");',
      '    expect(d->getArray("r_int32").size() == 3, "AllTypes repeated");',
      '    expect(std::get<double>(d->getObject("m_string_int").at("x")) == 9.0, "AllTypes map<string,int>");',
      '    expect(std::get<std::string>(d->getObject("m_int_string").at("7")) == "seven", "AllTypes map<int,string>");',
      '    expect(d->getString("o_string") == "pick" && d->getMap().count("o_int") == 0, "AllTypes oneof");',
      '  }',
      '',
      '  if (failures > 0) {',
